...
<section class="report-section">
    <div class="report-inner">
        ...
        <div class="prose">
            ...
            <p>IP lookups on the main tool are not logged. With JavaScript on (the normal case), the log text you paste stays in your browser; only the IP addresses pulled out of it are sent to our server, which looks them up and does not store them. With JavaScript off, the page falls back to sending the pasted text itself; the server pulls out the IP addresses, looks them up, and stores neither the text nor the addresses. The "Copy share link" button puts the current view's IP addresses into the link itself, after the <code>#</code>; that part of the link is never sent to our server and nothing about the link is stored, though when the recipient opens it their browser looks the IPs up the same way as a normal lookup. Visitor analytics are collected via a self-hosted instance of <a href="https://umami.is/" target="_blank" rel="noopener">Umami</a>, an open-source, privacy-friendly analytics platform that does not use cookies or share data with third parties.</p>
            ...
            <p>Until v5.0.0, ip2geo ran a community threat feed, the Community Block List, built from anonymized, opted-in contributions: CIDR network ranges and individual IP addresses classified as scanning or VPN/proxy infrastructure. Residential IP addresses were never collected. The feed was retired in v5.0.0: it no longer accepts contributions and is no longer published.</p>
            <p>No tokens, email addresses, or other personally identifiable information were included in the aggregate. The aggregate tables that remain contain only network ranges, IP addresses, and counts &mdash; with no link back to any individual contribution. Consent was opt-in only. Because contributed data was immediately aggregated with no user identifier retained, individual contributions cannot be separated from the aggregate after the fact; this is consistent with GDPR Recital 26, which excludes truly anonymized data from erasure requirements.</p>
